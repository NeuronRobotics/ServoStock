...
int IP_TransmitPkt( NET_PKT* sbfr )
{
	...
	if( !Que_IsQueued( &PendingARPQue, sbfr ) )
	{
		...
		sbfr->ARPTickStart = SystemTickGet();
		Que_AddTail( &PendingARPQue, sbfr );
		sbfr->PktFlags |= ARP_QUE_MASK;
	}
...
/*********************************************************************
 * Function:        void IP_FormatPacket( NET_PKT* sbfr, DWORD DestIP, BYTE protocol, WORD len )
 *
 * PreCondition:    None
 * Input:			sbfr - packet buffer
...
void IP_FormatPacket( NET_PKT* sbfr, DWORD DestIP, BYTE protocol, WORD len )
{
	...
	FormatEtherTxFrame( sbfr, ETH_PROTO_IP, SysMACAddr.v );
