...
void ICMP_RxHandler( NET_PKT *sbfr )
{
	...
	if( packet->Type == ICMP_ECHO_REQUEST )
	{
        ...
        src_ip = _arrayToDword(((IP_HEADER*)(sbfr->pNetworkLayer))->SourceAddr);
             			...
             			WORD seq)
{
	...
	NET_PKT* sbfr = AllocSocketBuffer( sizeof(ICMP_HEADER) + extraLen, 0, PKT_FLAG_TX|PKT_TX_AUTO_DEALLOC );
	...
	IP_FormatPacket( sbfr, DestIP, IP_PROT_ICMP, ICMPLen ); 
