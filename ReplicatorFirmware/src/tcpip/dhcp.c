...
// local protos
static void _DHCPAbort(void);
static int _DHCPReceive();
static void _DHCPSend(BYTE);
...
// local data
static WORD				_Xid[2];
static SOCKET			_dhcpSocket;
static int				_numOfAttempts;
static int				_retryDelay;
static WORD     		_DebugCntr;

static IP_ADDR      DHCPServerID;
static DWORD_VAL    DHCPLeaseTime_T2;
static DWORD_VAL    DHCPLeaseTime_T1;
static SM_DHCP      smDHCPState;

static IP_ADDR tempIPAddress;
static DWORD_VAL tempMask;
...
PUBLIC void DHCPTask(void)
{
    ...
    case SM_DHCP_INIT:
        ...
        ticksPerSec=SystemTickGetResolution();
    ...
    case SM_DHCP_RESET_WAIT:
        ...
        if ( _retryDelay != 0x0 )           
        {
            if ( TickGetDiff(SystemTickGet(), lastTryTick) >= 
                (ticksPerSec * ((rand() & 0xf) + _retryDelay)) )
            {
    ...
    case SM_DHCP_SELECTING:
        ...
        if ( TickGetDiff(SystemTickGet(), lastTryTick) >= DHCP_TIMEOUT*ticksPerSec )
        {
            /*
    ...
    case SM_DHCP_PREBOUND:
        ...
        else if ( DHCPRecvReturnValue == DHCP_ACK_MESSAGE )
        {
             ...
             * paramaters
             */
            TCPIPSetIPAddr(tempIPAddress.Val);
    ...
    case SM_DHCP_BOUND:
         ...
         */
        if ( TickGetDiff(SystemTickGet(), lastTryTick) >= ticksPerSec )
        {
...
/*********************************************************************
 * Function:        static void _DHCPAbort(void)
 *
...
/*********************************************************************
 * Function:        static int _DHCPReceive()
 *
 * PreCondition:    None
 *
 * Input:           None
...
static int _DHCPReceive()
{
    ...
    if ( pdhcpMsgHeader->Op == BOOT_REPLY )
    {
        ...
        pMacAddr=TCPIPGetMACAddrStr();

        for(i =0 ; i < 6; i++)
        {
            if( pdhcpMsgHeader->Chaddr[i] != pMacAddr->v[i])
                return type;
        }
        ...
        do
        {
            ...
            case DHCP_ROUTER:
                ...
                if ( len >= 4 )
                {
                    TCPIPSetIPGatewayByte(packet[i], packet[i+1], packet[i+2], packet[i+3]);
                    i+=4;
                }
                else
                    return DHCP_UNKNOWN_MESSAGE;
...
/*********************************************************************
 * Function:        static void _DHCPSend(BYTE messageTyp)
 *
 * PreCondition:    None
 *
...
static void _DHCPSend(BYTE messageType)
{
     ...
     */
    TCPIPGetMACAddr((MAC_ADDR*)dhcpPacket.dhcpHeader.Chaddr);
    for(i = 6 ; i < 16; i++)
    {
